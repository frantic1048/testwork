...
//进入主界面
	    ...
	    //点击上报时限
	    driver.findElement(By.css('#content > div.container-fluid > div > ul > li:nth-child(7) > a')).click();
	    //检查位置（ad_season.php）
	    expect(driver.getTitle()).to.eventually.equal('企业信息管理系统');
	    expect(driver.getCurrentUrl())
	      .to.eventually.equal(`${baseURL}/ad_season.php`)
	      .and.notify(done);
	    //点击添加
	    driver.findElement(By.css('#content > div.container-fluid > div > div:nth-child(2) > div > div.widget-content.nopadding > form > div.form-actions > button')).click();
	    //处理浏览器弹框
