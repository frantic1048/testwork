...
//检查添加通知并取消
	    //1、点击添加
	    driver.findElement(By.css('#content > div.container-fluid > div > div > a')).click();
	    //检查位置（ad_notice_insert.php）
	    expect(driver.getTitle()).to.eventually.equal('企业信息管理系统');
	    expect(driver.getCurrentUrl())
	    ...
	    //点击取消
		driver.findElement(By.css('#content > div.container-fluid > div > div > div > div.widget-content.nopadding > form > div.form-actions > button.btn.btn-warning')).click();
		//检查位置
		expect(driver.getTitle()).to.eventually.equal('企业信息管理系统');
	    expect(driver.getCurrentUrl())
	      .to.eventually.equal(`${baseURL}/ad_notice.php`)
...
//检查添加通知成功
	    ...
	    driver.findElement(By.css('textarea[name="notify_content"]')).sendKeys('TEST_CONTENT');
	    //点击添加
		driver.findElement(By.css('#content > div.container-fluid > div > div > div > div.widget-content.nopadding > form > div.form-actions > button.btn.btn-success')).click();
		//处理浏览器弹框
